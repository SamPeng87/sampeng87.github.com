...
## sds
如果这个robj的来源是以字符串的形式存储的，那就涉及到sds这样的结构，所谓sds,就是可变长字符串。
...
按上面的逻辑来看，如果`value >= LONG_MIN && value <= LONG_MAX`倒是好说，直接就是8。但如果超过了呢？sds需要占用的内存是字符串长度+(struct sdshdr)+1('\n'),也就是说假设key用一个uuid。也就是32位字符串。不好意思。这里最少就需要32+1+8=41字节.那就是8g内存。假设value值也是和key值一样的32位字符串。也就能个存8千万条左右的记录。如果位数更多呢？经常会把整个session仍进去。session字符串按200个字符串来算。key任然是一个32位的uuid那么，8g内存，再精简精简服务器和服务本身需要的内存，大概可以存放**2-3千万用户session**.对于一般应用而言，绰绰有余。
